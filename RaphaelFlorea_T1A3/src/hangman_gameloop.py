...
#A lot of words in program appears as list of characters, 
# so use this to convert back into word
...
"""Used for testing purposes, to print word to be guessed at start of game,
...
def word_generator(word_len, grand_list):
    ...
    #Word is selected only from portion of grand_list that has matching len
...
def letter_validator(guesses_left, guesses):
    ...
    while True:
        ...
        #Make sure letter is valid single alphabet character, 
        #and also is not nothing
...
def hint_system(guess_word, guesses):
    ...
    """Use sets to easily find remaining letters to guess,
        by finding letters that are in the guess word, 
        but not already guessed"""
...
#This function determines if letter is in word or not, 
# and prints out remarks accordingly
def guess_checker(letter, guess_word, word_shown, 
...
def hangman_loop(guess_word, stages, word_len):
    ...
    while guesses_left > 0:
        ...
        # provided hint not yet given
        if guesses_left == 1 and not hint_given:
            hint_system(guess_word, guesses)
            
            #Use this variable to show program hint has been given, 
            # so it can't be given again
            hint_given = True
